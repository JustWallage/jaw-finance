...
# Access Application – protects staging.jaw-finance.pages.dev for E2E tests
resource "cloudflare_zero_trust_access_application" "jaw_finance_staging" {
  account_id                = var.cloudflare_account_id
  name                      = "jaw-finance-staging"
  domain                    = "staging.jaw-finance.pages.dev"
  type                      = "self_hosted"
  session_duration          = "24h"
  auto_redirect_to_identity = false
...
# Service token for E2E tests – authenticates Playwright against staging
resource "cloudflare_zero_trust_access_service_token" "e2e" {
  account_id = var.cloudflare_account_id
  name       = "jaw-finance-e2e"
  duration   = "8760h"
}
