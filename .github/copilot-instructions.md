...
- **Authentication:** Cloudflare Access (Zero Trust) protects the application UI.
  ...
  * In E2E tests, Playwright sends `Cf-Access-Authenticated-User-Email` via `extraHTTPHeaders` locally. In CI, it authenticates via CF Access Service Token (`CF-Access-Client-Id` / `CF-Access-Client-Secret`) and CF Access sets the email header automatically.
