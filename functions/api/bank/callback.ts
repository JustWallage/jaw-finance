...
export const onRequestGet: PagesFunction<EBEnv> = async (context) => {
  ...
  if (error) {
    ...
    return Response.redirect(`${url.origin}/?bank_error=${encodeURIComponent(desc)}`, 302);
  }

  if (!code) {
    return Response.redirect(`${url.origin}/?bank_error=${encodeURIComponent("Missing authorization code")}`, 302);
  }
  ...
  try {
    ...
    for (const account of data.accounts) {
      ...
      await env.DB.prepare(
        `INSERT INTO bank_connections (session_id, account_uid, aspsp_name, aspsp_country, iban, valid_until)
         VALUES (?, ?, ?, ?, ?, ?)
